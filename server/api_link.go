...
func (s *ApiServer) LinkMetamask(ctx context.Context, in *api.AccountMetamask) (*emptypb.Empty, error) {
	userID := ctx.Value(ctxUserIDKey{}).(uuid.UUID)

	// Before hook.
	if fn := s.runtime.BeforeLinkMetamask(); fn != nil {
		beforeFn := func(clientIP, clientPort string) error {
			result, err, code := fn(ctx, s.logger, userID.String(), ctx.Value(ctxUsernameKey{}).(string), ctx.Value(ctxVarsKey{}).(map[string]string), ctx.Value(ctxExpiryKey{}).(int64), clientIP, clientPort, in)
			if err != nil {
				return status.Error(code, err.Error())
	...
	err := LinkMetamask(ctx, s.logger, s.db, s.config, s.socialClient, userID, in.Address, in.Signature)
	if err != nil {
		return nil, err
	}

	// After hook.
	if fn := s.runtime.AfterLinkMetamask(); fn != nil {
		afterFn := func(clientIP, clientPort string) error {
			return fn(ctx, s.logger, userID.String(), ctx.Value(ctxUsernameKey{}).(string), ctx.Value(ctxVarsKey{}).(map[string]string), ctx.Value(ctxExpiryKey{}).(int64), clientIP, clientPort, in)
		}
...
func (s *ApiServer) UnlinkMetamask(ctx context.Context, in *api.AccountMetamask) (*emptypb.Empty, error) {
	userID := ctx.Value(ctxUserIDKey{}).(uuid.UUID)
