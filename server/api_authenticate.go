...
func (s *ApiServer) AuthenticateMetamask(ctx context.Context, in *api.AuthenticateMetamaskRequest) (*api.Session, error) {
	// Before hook.
	if fn := s.runtime.BeforeAuthenticateMetamask(); fn != nil {
	...
	if in.Account == nil || in.Account.Address == "" || in.Account.Signature == "" {
		return nil, status.Error(codes.InvalidArgument, "Metamask address and signature are required.")
	}

	// Validate the signature
	valid, err := verifySignature(in.Account.Address, in.Account.Signature)
	if err != nil {
	...
	dbUserID, dbUsername, created, err := AuthenticateMetamask(ctx, s.logger, s.db, s.socialClient, in.Account.Address, in.Account.Address, in.Account.Signature, create)
	if err != nil {
		return nil, err
	}
	...
	// After hook.
	if fn := s.runtime.AfterAuthenticateMetamask(); fn != nil {
